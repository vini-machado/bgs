...
def generate_report(report_data: dict, archives: list = [], template_file: str = TEMPLATE_FILE):
    ...
    date_values_for_report = {
        ...
        'measurement_date': dataframe['measurement_date'].unique().min().strftime("%d/%m/%Y")
    }
...
def write_dataframe(filename: str, workbook: openpyxl.Workbook, dataframe: pd.DataFrame):

    dataframe = rename_columns(dataframe).reset_index()

    if 'Diâmetro Fundo (mm)' in dataframe.columns:
        pivot_df = pd.pivot_table(data=dataframe, index='Profundidade (mm)', columns='Nome do Ensaio', values=['Diâmetro Cheio (mm)', 'Diâmetro Fundo (mm)']).T
    else:
        pivot_df = pd.pivot_table(data=dataframe, index='Nome do Ensaio', columns='Profundidade (mm)', values='Diâmetro Cheio (mm)')
